...
XBB v5.0.0 and up are more modular, using binary xpm packages.

### GNU/Linux

There are two Docker images (x64 and arm64) with Debian 10,
basic development tools and node/npm.

The toolchains and other tools are installed as binary xpm development
dependencies.

- https://xpack-dev-tools.github.com/

### macOS

For macOS the only prerequisite is the Command Line Tools (CLT) package.

`realpath` is provided as a separate binary xpm package.

`tree` is compiled from sources; in the future it should be a separate
xpm package.

## Future work

The current Debian 10 base is expected to last 1-2 years.

The next major moment might be 2027, when RedHat 9 will enter extended support.

The next probable upgrade will be to Debian 11; however, the date for this
upgrade remains to be decided.
