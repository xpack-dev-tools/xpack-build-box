...
With the environment properly set, the command to install **xpack-build-box** is:

```sh
npm install --location=global xpack-build-box@latest
```
...
`npm install --location=global xpack-build-box@${customField('releaseVersion')}
`} </CodeBlock>
...
The result of installing **xpack-build-box** is a set of files in the `%APPDATA%\npm` folder:
...
The command to remove **xpack-build-box** is:

```sh
npm uninstall --location=global xpack-build-box
```
...
If, for any reason, you decide not to install **xpack-build-box**, you can still
